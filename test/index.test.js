...
describe("When esm-webpack-plugin is invoked", () => {
    ...
    it("should import esm externals", async () => {
        ...
        assert.strictEqual(module.externals.foo.default, 'foo external')
        assert.strictEqual(module.externals.bar.default, 'bar external')
    });
    ...
    it("should add __esModule to externals, with esModuleExternals option set", async () => {
        const buildResults = await build;
        const module = await buildResults.external_esmodule.import();
        assert.strictEqual(module.externalsHaveEsModule(), true);
    });
});
